...
export function AccountPage() {
  ...
  const [activeSection, setActiveSection] = useState<AccountSection>('settings');
  const [hoveredSection, setHoveredSection] = useState<SidebarItemKey | null>(null);
        ...
        <section className="account-content" ref={contentRef}>
          {activeSection === 'orders' && (
            <div className="account-block account-scroll-item">
              {ordersLoading ? (
                ...
                <p className="account-error">Failed to load orders</p>
              ) : orders.length === 0 ? (
                <div className="account-empty account-scroll-item">
                  <p className="account-muted">
                    OH! You have no orders yet. Choose an event you want to visit.
                  </p>
                  <Link to="/" className="account-home-link">
                    <span>Go to Home Page</span>
                    <img src={homeArrowIcon} alt="" className="account-home-link__icon" />
                  </Link>
                </div>
              ) : (
                <div className="orders-list">
                  ...
                  {orders.map((order) => (
                    <div className="orders-item account-scroll-item" key={order.id}>
                      <div>{order.event?.title ?? 'Event not found'}</div>
                      <div>{order.quantity}</div>
                      <div>{formatOrderDate(order.event?.startsAt, order.event?.endsAt)}</div>
                      <div>{formatOrderPrice(order.totalPrice)}</div>
                      <div>{order.event?.placeAddress ?? '-'}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activeSection === 'events' && (
            <div className="account-block account-scroll-item">
              {!companyLoading && !company ? (
                <div className="account-empty account-scroll-item">
                  <p className="account-muted">
                    OH! You have no events yet. First, create your company to be able to add an event.
                  </p>
                  <Link
                    to="/create-company"
                ...
                <div className="account-empty account-scroll-item">
                  <p className="account-muted">
                    OH! You have no events yet. Create your first event to get started.
                  </p>
          ...
          {activeSection === 'admin' && (
            <div className="account-block account-scroll-item">
              ...
              <div className="admin-section">
                <h3 className="admin-section__title">Companies</h3>
                {adminCompaniesLoading ? (
                  <p className="account-muted">Loading...</p>
                    ...
                    {adminCompanies.map((item) => (
                      <div key={item.id} className="admin-row">
                        <div className="admin-row__info">
                          ...
                          <p><strong>Email:</strong> {item.email}</p>
                          <p><strong>Address:</strong> {item.placeAddress ?? '-'}</p>
                        </div>

                        <button
                          className="account-danger-btn"
                          onClick={() => handleAdminDeleteCompany(item.id)}
                        >
                          Delete
                        </button>
                      </div>
                    ...
                    {adminEvents.map((item) => (
                      <div key={item.id} className="admin-row">
                        <div className="admin-row__info">
                          ...
                          <p><strong>Place:</strong> {item.placeAddress ?? item.placeName}</p>
                        </div>

                        <button
                          className="account-danger-btn"
                          onClick={() => handleAdminDeleteEvent(item.id)}
                        >
                          Delete
                        </button>
                      </div>
              ...
              <div className="admin-section">
                <h3 className="admin-section__title">Comments</h3>
                {adminCommentsLoading ? (
                  <p className="account-muted">Loading...</p>
                  ...
                  <div className="admin-list">
                    {adminComments.map((item) => (
                      <div key={item.id} className="admin-row">
                        <div className="admin-row__info">
                          <p><strong>Comment:</strong> {item.content}</p>
                          <p><strong>Event ID:</strong> {item.eventId}</p>
                          <p><strong>Author ID:</strong> {item.authorUserId}</p>
                        </div>
          ...
          {activeSection === 'settings' && (
            <>
              <div className="account-block account-scroll-item">
                <h2 className="account-block__heading">LOGIN</h2>
                ...
                {settingsError ? (
                  <p className="account-error">{settingsError}</p>
                ) : null}
              </div>

              <div className="account-block account-scroll-item">
                <h2 className="account-block__heading">COMPANY</h2>
                  ...
                  <>
                    <div className="company-card account-scroll-item">
                      <div className="company-card__avatar">
                        <img
                          src={company.avatarUrl || companyDefaultAvatar}
                          alt={company.name}
                          onError={(e) => {
                            e.currentTarget.onerror = null;
                            e.currentTarget.src = companyDefaultAvatar;
                          }}
                        />
                      ...
                      <div className="company-card__fields">
                        ...
                        <input
                          ...
                          value={companyForm.placeAddress}
                          disabled={!isEditingCompany}
                          onChange={(e) =>
                            setCompanyForm((prev) => ({
                              ...prev,
                    ...
                    {companyError ? (
                      <p className="account-error">{companyError}</p>
                    ) : null}
                  </>
                )}
              </div>
